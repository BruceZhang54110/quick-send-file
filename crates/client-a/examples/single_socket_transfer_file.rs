...
async fn main() -> Result<()> {
    ...
    let (mut send_stream, mut recv_stream) = c1.open_bi().await?;
    send_stream.write_all(b"test").await?;
    let mut bytes = vec![0, 10];
    send_stream.write_chunks(bytes).await?;
    send_stream.finish()?;


    let recv_byte = recv_stream.read_to_end(10).await?;
